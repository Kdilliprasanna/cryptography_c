/* Experiment 36: Affine Cipher -- generalization of the Caesar cipher.
   (Same construction as Experiment 5, repeated as its own exercise.)
   C = E([a,b], p) = (a*p + b) mod 26   ,   p = D([a,b], C) = a^-1*(C-b) mod 26
   The cipher is one-to-one only when gcd(a,26)=1; b has no restriction. */
#include <stdio.h>
#include <string.h>
#include <ctype.h>
