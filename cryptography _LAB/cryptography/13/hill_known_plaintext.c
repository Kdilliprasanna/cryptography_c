/* Experiment 13: Known-Plaintext Attack on the Hill Cipher
   Given TWO plaintext-ciphertext digraph pairs (P1,C1) and (P2,C2), we can
   recover the 2x2 key matrix K, because  C = K * P (mod 26)  implies
   K = C * P^-1 (mod 26), where P is the 2x2 matrix formed from the two
   known plaintext digraphs (as columns) and C is formed the same way from
   the corresponding ciphertext digraphs. */
#include <stdio.h>
#include <ctype.h>
